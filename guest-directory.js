const GUEST_DIRECTORY_API = 'https://script.google.com/macros/s/AKfycbxKSsN3D86XzXPVqdNNpSbjU87y7uhgpg4DH-RKlVpLm6Xfo5DCPKw5yFsWjpACqNbUlw/exec';
...
function buildCard(guest, index) {
  ...
  const label = guest.isNpc ? 'HOTEL RECORD / NPC' : `GUEST RECORD / ${String(index + 1).padStart(2, '0')}`;
  card.type = 'button';
  card.className = 'guest-record-card';
  card.innerHTML = `
    <span class="guest-record-label">${label}</span>
    <span class="guest-record-image">${imageUrl ? `<img src="${safeText(imageUrl)}" alt="${name} 초상">` : '<span>GUEST<br>PORTRAIT</span>'}</span>
    <strong>${name}</strong>
...
function setProfilePortrait(guest) {
  ...
  const imageUrl = guestImageUrl(guest.img);
  portrait.innerHTML = imageUrl ? `<img src="${safeText(imageUrl)}" alt="${safeText(guest.name)} 초상">` : '<span>GUEST<br>PORTRAIT</span>';
}
...
async function openGuestProfile(name) {
  ...
  document.querySelector('#guest-profile-type').textContent = guest.isNpc ? 'HOTEL RECORD / NPC' : 'THEOBROMA / GUEST RECORD';
...
async function loadGuestDirectory() {
  try {
    ...
    const data = await response.json();
    if (!data.success || !Array.isArray(data.list) || data.list.length === 0) throw new Error('등록된 투숙객 기록이 없습니다.');
    guestGrid.innerHTML = '';
    data.list.forEach((guest, index) => {
      guestCache.set(guest.name, guest);
      guestGrid.append(buildCard(guest, index));
    });
    guestCount.textContent = `GUEST RECORDS / ${String(data.list.length).padStart(2, '0')}`;
  } catch (error) {
    guestGrid.innerHTML = `<p class="directory-error">${safeText(error.message || '투숙객 기록을 불러오지 못했습니다.')}</p>`;
    guestCount.textContent = 'GUEST RECORDS / ERROR';
  }
}
